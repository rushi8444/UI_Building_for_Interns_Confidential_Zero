...
lines = open('found_file.txt', 'r', encoding='utf-8').read().split('\n')
if lines[0].endswith('ColumnFunctions.cpp'):
    lines = lines[1:]
...
# Base includes and externs
base_code = """// clang-format off
#include "stdafx.h"
// clang-format on
#include <atomic>
#include <string>
...
new_code = "\n".join([base_code, l2_structs, structs_l1, helpers_l1, class_l1, merged_init, exports])
...
new_code = new_code.replace(patch_1, patch_1_new)

patch_2 = """        // DO NOT call SetValue to prevent UI redraw lag!
        return false;
    }"""
patch_2_new = """        char newText[128];
...
new_code = new_code.replace(patch_2, patch_2_new)

patch_3 = """        char newText[128];
        sprintf_s(newText, sizeof(newText), "%llu", vol);
        SetValue(newText);
        return false;
    }"""
new_code = new_code.replace(patch_3, patch_2_new)
